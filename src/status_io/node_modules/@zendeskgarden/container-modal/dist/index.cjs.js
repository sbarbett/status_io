...
const useModal = _ref => {
  ...
  const isModalMousedDownRef = React.useRef(false);
  ...
  const getBackdropProps = function (_temp) {
    ...
    return {
      onMouseUp: containerUtilities.composeEventHandlers(onMouseUp, handleMouseUp),
  ...
  const getModalProps = function (_temp2) {
    ...
    return {
      ...
      onMouseDown: containerUtilities.composeEventHandlers(onMouseDown, () => {
  ...
  const getCloseProps = _ref2 => {
    ...
    return {
      onClick: containerUtilities.composeEventHandlers(onClick, event => {
        closeModal(event);
      }),
      ...other
    };
  ...
  const {
    getContainerProps
  } = containerFocusjail.useFocusJail({
